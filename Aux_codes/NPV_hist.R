...
aggtab = read.csv("Output/CRV_his.csv") %>% add_yearnum() %>%
  rename(CRVl2 = CRVn) 
...
interpolate_df <- function(df, syear, eyear) {
  ...
  df %>% left_join(newyearsnum,., by = "yearnum") %>%
    mutate(#CRVl = na.approx(CRVl, maxgap = Inf, rule = 2),
           CRVl2 = na.approx(CRVl2, maxgap = Inf, rule = 2),
           dEDD = na.approx(dEDD, maxgap = Inf, rule = 2),
           soyarea = na.approx(soyarea, maxgap = Inf, rule = 2),
           areasoyp = na.approx(areasoyp, maxgap = Inf, rule = 2),
           Floss = na.approx(Floss, maxgap = Inf, rule = 2),
           dy = na.approx(dy, maxgap = Inf, rule = 2),
...
write.csv(npvaggtib,paste0("Output/CRV_hist_new",drate,".csv"))
